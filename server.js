...
if (state.action === "await_change_bank") {
  if (!text.includes("|")) {
    return ctx.reply(
      "⚠️ Invalid format.\nPlease send: oldBank,oldAccNumber,oldName | newBank,newAccNumber,newName"
    );
  }

  const [oldStr, newStr] = text.split("|").map((s) => s.trim());
  const [oldBank, oldAccNum, oldAccName] = oldStr.split(",").map((s) => s.trim());
  const [newBank, newAccNum, newAccName] = newStr.split(",").map((s) => s.trim());
  const userId = ctx.from.id;

  try {
    const user = await pool.query(
      "SELECT bank_name, account_number, account_name FROM users WHERE telegram_id = $1",
      [userId]
    );
    ...
    if (
      ...
      u.account_number !== oldAccNum ||
      u.account_name.toLowerCase() !== oldAccName.toLowerCase()
    ) {
      await ctx.reply(
        "🚫 The old bank details you entered do not match our records. Update request denied."
      );
      userState.delete(userId);
      return;
    }
    ...
    await pool.query(
      ...
      [newBank, newAccNum, newAccName, userId]
    );
